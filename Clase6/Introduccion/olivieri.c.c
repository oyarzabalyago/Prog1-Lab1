...
/** \brief MUESTRA CHAR DE ARRAY
 * \param RECIBE EL CHAR ARRAY
 * \param RECIBE EL TAMAÑO
 * \return MUESTRA EL CHAR ARRAY
 */
void mostarCharArray (char vec[], int tam)
{
    for (int i = 0;i < tam;i++)
    {
        printf("%c  ", vec[i]);
 ...
 * \param EL TAMAÑO DEL VECTO
 * \param CRITERIO DE ORDENAMIENTO
 * \return EL VECTOR ORDENADO
 */
int burbujeoEnteros (int vec[], int tam, int criterio)
{
    int aux;
    if (criterio == 1)
    {
...
/** \brief ORDENA UN ARRAY DE CARACTERES
 * \param RECIBE EL ARRAY
 * \param REIBE EL TAMAÑO
 * \param CRITERIO DE ORDENAMIENTO
 * \return LO DEVUELVE ORDENADO
 */
int burbujeoChar (char vec[], int tam, int criterio)
{
    char aux;
    if (criterio == 1)
    {
